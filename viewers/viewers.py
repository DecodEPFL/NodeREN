...
def viewContour2D_contraction(domain, model, input_ch=None,method ="euler",time_vector = None, device="cpu"):
    '''
    Coloured regions in domain represent the prediction of the DNN.
    For 2d datasets.
    '''
...
def viewPropagation_circles_contraction(dataset, labels, model,method="euler",time_vector=torch.empty(1),device="cpu"):
    ...
    # values = torch.cat((x.flatten().unsqueeze(1),y.flatten().unsqueeze(1)),1)
    # z = model.output(time_vector[-1],values)
    # z = z - 0.5*torch.ones(z.shape,device=device)
    # plt.contour(x.flatten().unsqueeze(1).cpu(), y.flatten().unsqueeze(1).cpu(), z.cpu(),
                # [0], linestyles={'dashed'})
