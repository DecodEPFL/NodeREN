...
class System_contractive(nn.Module):
    def __init__(self, nx, ny, nu, nq, sigma, epsilon, device, bias = False, alpha = 0.0, linear_output=False):
        """Used by the upper class REN_ODE to guarantee contractivity to the model. It should not be used by itself.
        Args:
        ...
        self.device = device
        std = .1           #standard deviation used to draw randomly the initial weights of the model.
    ...
    def updateParameters(self):
        ...
        self.Lambda = Lambda                                            #TODO ELIMINARE
        self.Y_alpha_0 = -0.5*(H1 + self.Y1-self.Y1.T)                  #TODO ELIMINARE
    ...
    def calculate_w(self,t,xi,u):
        ...
        It is used by the module REN_ODE.calculate_w_Vdot_s_vectors().
        """
        n_initial_states = xi.shape[0]
        vec = torch.zeros(self.nq,1,device=self.device)
        vec[0,0] = 1.
...
class System_robust_passive_input(nn.Module):
    def __init__(self, nx, ny, nu, nq,sigma, epsilon, S, Q, R, ni,device, bias = False, alpha=0.0):
        """Used by the upper class REN_ODE to guarantee the model to be (incrementally) input passive (and thus robust). It should not be used by itself.
    ...
    def updateParameters(self):
        ...
        # pr11 = -Y -Y.T
        # pr12 = -Z - self.Chi  
        # pr13 = -V+ F.linear(self.C2.T,self.S)
        # pr21= pr12.T
        # pr22=H22
        # pr23=F.linear(self.D21.T,self.S)-T
        # pr31=pr13.T
        # pr32=pr23.T
        # pr33=self.R + F.linear(self.S,self.D22.T)+F.linear(self.D22.T,self.S)
        # LMI = torch.cat([
        #     torch.cat([pr11,pr12,pr13],1),
        #     torch.cat([pr21,pr22,pr23],1),
        #     torch.cat([pr31,pr32,pr33],1)],0)
...
class System_robust_passive_output(nn.Module):
    def __init__(self, nx, ny, nu, nq,sigma, epsilon, S, Q, R, rho,device, bias = False, alpha = 0.0):
        """Used by the upper class REN_ODE to guarantee the model to be (incrementally) output passive (and thus, robust). It should not be used by itself.
...
class System_robust_L2_bound(nn.Module):
    def __init__(self, nx, ny, nu, nq,sigma, epsilon, S, Q, R, gamma,device, bias = False, alpha = 0.0):
        """Used by the upper class REN_ODE to guarantee the model to be L2 Lipschitz bounded in its input-output mapping (and thus, robust). It should not be used by itself.
...
class Controller(nn.Module):
    def __init__(self, nu,device="cpu"):
        """Base class used by REN_ODE to apply an input function u(t) described by the forward() method. It should not be used by itself.
...
class REN_ODE(nn.Module):
    def __init__(self, nx = 5, ny = 5, nu = 5, nq = 5, sigma = "tanh", epsilon = 1.0e-2, mode = "c", gamma = 1., device = "cpu", bias = False, ni = 1., rho = 1., alpha = 0.0, linear_output=False):
        """Base class for Recurrent Equilbrium Network Ordinary Differential Equations (REN_ODEs).
        ...
        """
        super().__init__()
        self.ctl = Controller(nu,device)
        self.mode = mode.lower()
        if (self.mode == "c"):
            self.sys = System_contractive(nx, ny, nu, nq,sigma, epsilon, device=device,bias=bias, linear_output=linear_output,alpha=alpha)
        elif(self.mode == "rl2"):
            Q = -(1./gamma)*torch.eye(ny,device=device)
            R = (gamma)*torch.eye(nu,device=device)
            S = torch.zeros(nu,ny,device=device)
            self.sys = System_robust_L2_bound(nx, ny, nu, nq,sigma, epsilon,S=S,Q=Q,R=R,gamma=gamma,device=device, bias = bias,alpha=alpha)
            ...
            self.sys = System_robust_passive_input(nx, ny, nu, nq,sigma, epsilon,S=S,Q=Q,R=R,ni=ni,device=device, bias = bias,alpha=alpha)
            ...
            S = torch.eye(ny,device=device)
            self.sys = System_robust_passive_output(nx, ny, nu, nq,sigma, epsilon,S=S,Q=Q,R=R,rho=rho,device=device, bias = bias,alpha=alpha)
        else:
    ...
    #     if number == 2:
    #         self.ctl = Controller_2(nu)
    #     if number == 3:
    #         self.ctl = Controller_3(nu)
